...
export class PostProjectsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.missionService.getHiredMissionsForClient(sessionStorage.getItem('id'))
      .subscribe(missions => {
  ...
  setImage(mission) {
    const image = mission.freelancer.image ? mission.freelancer.image : '';


      return 'data:image/jpeg;base64,' + mission.freelancer.image;
